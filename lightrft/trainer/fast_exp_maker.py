...
class FastExperienceMaker(NaiveExperienceMaker):
    ...
    def __init__(self, *args, packing_samples: bool = False, processor=None, **kwargs):
        ...
        # Initialize advantage estimator / calculator
        self.advantage_estimator = self.strategy.config.advantage_estimator
        # GSPO uses a custom advantage computation path, so we don't need a calculator instance
        if self.advantage_estimator == "gspo":
            self.advantage_calculator = None
        else:
            self.advantage_calculator = get_advantage_calculator(
                self.advantage_estimator, self.strategy.config
            )
    ...
    def _process_experiences(
        ...
        # GSPO uses a custom reward preprocessing path; other estimators delegate to
        # the unified AdvantageCalculator.
        if config.advantage_estimator == "gspo":
        ...
        # For all other estimators, use the calculator's preprocess_rewards method
